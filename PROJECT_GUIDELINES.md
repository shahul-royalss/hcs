...
| Second backend: `frontend/public/api/` (PHP 8 + SQLite, single-file) | Mirrors the FastAPI routes/response shapes needed by the site + admin (auth incl. change-password, bookings, inquiries, testimonials, staff, patients, gallery, analytics) so the identical frontend runs on Hostinger shared hosting with data in `api/data/dhrishta.sqlite` (web-denied dir, gitignored). AI chat/payments/SMS answer 503 there. | Shared hosting cannot run Python/Mongo; the owner wanted the admin working on their existing Hostinger plan. FastAPI remains the canonical backend — when changing API shapes, update BOTH or gate the feature. |
